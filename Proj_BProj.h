...
template<typename mTYPE1,typename mTYPE2>
void pBackProj_simpl(const MATRIX<mTYPE1>*,MATRIX<mTYPE2>&,int);//検出確率計算が単純なもので単純逆投影を行う

template<typename mTYPE>
MATRIX<mTYPE> ConvInte(double (*conv_func)(int,double),const MATRIX<mTYPE>&,double);//サイノグラムに対して指定したフィルタで重畳積分をかける
template<typename mTYPE>
MATRIX<mTYPE> pConvInte(double (*conv_func)(int,double),const MATRIX<mTYPE>*,double);//サイノグラムに対して指定したフィルタで重畳積分をかける//ポインタを引数にするver.

double Shepp_Logan_cf(int,double);//指定されるフィルタのひとつ。Shepp Logan convolution filter
double Ram_Lak_cf(int,double);//Ram_Lakフィルタ//これは未完成
double Chesler_cf(int,double);//Cheslerフィルタ//これは未完成
...
template<typename mTYPE>
MATRIX<mTYPE> pdevide_each(const MATRIX<mTYPE>*);//投影方向ごとの最大値で各投影を割った行列を返す。ポインタを引数とする。
...
template<typename mTYPE1,typename mTYPE2>
void pBackProj_simpl(const MATRIX<mTYPE1> *sinogram,MATRIX<mTYPE2> *image,int scan)	//ポインタを引数とするver.
  /*
  scan=1でハーフスキャン再構成。scan=2でフルスキャン再構成。
  ピクセルの投影幅を1ステップ分と見なして検出確率を計算する単純逆投影プログラム。simpleと呼ぶことにする。
  sinogramは行が投影方向、列がステップ数となるように準備すること。線減弱係数の投影の和(-log(I/I0))にしておくこと。
...
template<typename mTYPE>
MATRIX<mTYPE> pConvInte(double (*conv_func)(int,double),const MATRIX<mTYPE> *sinogram,double step_interval)	//ポインタを引数とするver.
{
  /*
  コンボリューションを行う関数。
  引数にコンボリューション関数(サブ関数の名前の部分)を与えると、これを使って計算する。
...

{
	...
	if(pixel_distance%2==1){//奇数
		...
		h=-1/(pixel_distance*pixel_distance*M_PI*step_interval*step_interval);
		return h;
...
//Cheslerフィルタ//これも未完成
double Chesler_cf(int pixel_distance,double step_interval)
//ConvInteに使うコンボリューション関数。
//pixel_distanceはピクセル数で数えたときの、注目ステップと影響を考えるステップとの距離。
//step_intervalは投影ステップのステップ間距離(cm)。ここの単位がcmなので、最終的に再構成画像の単位が(1/cm)になる
//Cheslerフィルタについても、調べると係数の付き方が様々で混乱するかもしれない。
//Shepp_Logan_cfと同様に逆投影のプログラムとの都合を考え、下記の通りに計算する。2017/1/12/ishikawa

{
	double h;
	double buf;

	if(pixel_distance==0){//0のとき
		buf=1/4-1/(M_PI*M_PI);
		h=M_PI*buf/(2*step_interval*step_interval);
		return h;
	}else if(pixel_distance==1||pixel_distance==-1){//1 or -1 のとき
		buf=1/8-1/(M_PI*M_PI);
		h=M_PI*buf/(2*step_interval*step_interval);
		return h;
	}else if(pixel_distance%2==0){//0以外の偶数のとき
		buf=(pixel_distance*pixel_distance-1)*(pixel_distance*pixel_distance-1);
		h=(pixel_distance*pixel_distance+1)/(2*M_PI*step_interval*step_interval*buf);
		return h;
	}else{//1, -1以外の奇数のとき
		h=1/(2*M_PI*step_interval*step_interval*pixel_distance*pixel_distance);
		return h;
	}
}
  ...
  for(k=0; k<Num_angle;k++)
  {
    ...
    for(i=0;i<Height;i++)
    {
      ...
      for(j=0;j<Width;j++)
      {
        ...
        //格納
        DP_ixx[J+j]=ixx;
        DP_LCR[0][J+j]=dp_LCR[0];
        DP_LCR[1][J+j]=dp_LCR[1];
        DP_LCR[2][J+j]=dp_LCR[2];
      }
    }
  }
...
void transmission_distance(const DP_trapezoid& DP,const MATRIX<short>& image,int Num_component,double pix_size,MATRIX<mTYPE>& TD)
{
  ...
  float *DP_LCR[3];//DPをdoubleにするならここを対応させること。
  DP_LCR[0]=DP.dp_L.pVALUE();
  DP_LCR[1]=DP.dp_C.pVALUE();
  DP_LCR[2]=DP.dp_R.pVALUE();
...
template<typename mTYPE>
MATRIX<mTYPE> pdevide_each(const MATRIX<mTYPE>* sinogram)//投影方向ごとの最大値で各投影を割った行列を返す。ポインタを引数とする。
{
  int buf_row=sinogram->Row();
  int buf_col=sinogram->Col();
